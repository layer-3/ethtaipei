import { toSignificant } from './precision';
...
export const formatSignificantWithSeparators = (value: string, thousSep: string = ',', floatSep?: string) => {
    const fmtNum = toSignificant(String(value), SIGNIFICANT_DIGITS);

    return formatWithSeparators(fmtNum, thousSep, floatSep);
};

class Decimal extends React.Component<DecimalProps> {
    public static format(
        value: DecimalProps['children'],
        precision: number | undefined = 0,
        thousSep?: string,
        floatSep?: string,
    ) {
        ...
        if (fmtVal !== '' && fmtVal !== 0) {
            result = handleRemoveExponent(
                parseFloat(`${Math.floor(Number(`${handleRemoveExponent(fmtVal)}e${precision}`))}e-${precision}`),
            );
        }
    ...
    public render() {
        const { children, fixed, prevValue, thousSep, floatSep, highlightedPartialyEnabled, secondColor } = this.props;
    ...
    private highlightNumbers = (
        ...
        return (
            <React.Fragment>
                <span data-testid="number" style={{ color: cxSecondColor || this.props.color }}>
                    {val}
                </span>
                <span data-testid="decimal" style={{ color: this.props.color }}>
                    {highlighted}
                </span>
            </React.Fragment>
        );
